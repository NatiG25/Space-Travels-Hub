import missionReducer, { joinMission, cancelMission, getMissions } from '../components/redux/missions/missions';
...
describe('Should return a new state with a reserved property', () => {
  ...
  it('Should update the state using the missionID to reserved: true ', () => {
    expect(missionReducer(allMissions, joinMission(missionID)))
      .toEqual(allMissions.map((item) => (item.mission_id === missionID
        ? { ...item, reserved: true }
        : item)));
  });

  it('Should update the state using the missionID to reserved: false ', () => {
    expect(missionReducer(allMissions, cancelMission(missionID)))
      .toEqual(allMissions.map((item) => (item.mission_id === missionID
        ? { ...item, reserved: false }
        : item)));
  });
